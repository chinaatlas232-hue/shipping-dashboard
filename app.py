...
# 🔍 **الفلتر الجانبي الذكي المطلوب (يعمل بعد الضغط على Enter)**
with st.sidebar.form(key="sidebar_code_search_form"):
  sidebar_code_search = st.text_input(
      "🔍 بحث الكود الجانبي:",
      value="",
      placeholder="اكتب الكود واضغط Enter...",
  ).strip()
  st.form_submit_button(label="بحث")
...
# تطبيق فلتر الكود الجانبي الذكي أولاً إذا أدخل المستخدم قيمة
if sidebar_code_search:
  filtered_df = apply_strict_code_search(filtered_df, sidebar_code_search)

if container_col:
  containers = ["الكل"] + sorted(
      filtered_df[container_col].dropna().astype(str).unique().tolist()
  )
