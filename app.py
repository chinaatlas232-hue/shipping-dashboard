...
# --- 3. قراءة البيانات (بيانات افتراضية مطابقة لجدولك تماماً) ---
@st.cache_data
def load_data(file):
    if file is not None:
...
# جلب الاسم الحقيقي لعمود الكونتينر من ملفك
...
ctns_col = "Sum of Ctns" if "Sum of Ctns" in df.columns else ("Cartons" if "Cartons" in df.columns else None)
cbm_col = "Sum of Cbm" if "Sum of Cbm" in df.columns else ("Volume_CBM" if "Volume_CBM" in df.columns else None)
...
# الحسابات الصحيحة والديناميكية بناءً على الفلتر
if selected_container != "الكل":
    ...
    # عند اختيار الكل، يعرض إجمالي عدد الصفوف كعدد طلبات وإجمالي الحاويات الفريدة
    total_orders = int(df["Orders"].sum()) if "Orders" in df.columns else len(df)
    total_containers = df[container_col].nunique()
